...
if [ -f "rlw-core.sh" ]; then
	...
	if command -v zenity >/dev/null 2>&1 ; then
		zenity \
		 ...
		 kdialog --icon roblox --error "Missing rlw-core: try reinstalling rlw using the main script. If this problem presists, please report an issue to our GitHub page.\n"  --title "Roblox Linux Wrapper" 2&> /dev/null
	fi
    exit 1
fi
